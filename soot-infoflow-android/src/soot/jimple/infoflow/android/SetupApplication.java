...
public class SetupApplication {
	...
	public final Map<String, Set<AndroidMethod>> callbackMethods = new HashMap<String, Set<AndroidMethod>>(10000);
	...
	//szy
	public AndroidSourceSinkManager sourceSinkManager = null;
	...
	public void calculateSourcesSinksEntrypoints2
			(String sourceSinkFile) throws IOException, XmlPullParserException {
		PermissionMethodParser parser = PermissionMethodParser.fromFile(sourceSinkFile);
		Set<AndroidMethod> sources = new HashSet<AndroidMethod>();
		Set<AndroidMethod> sinks = new HashSet<AndroidMethod>();
	...
	//szy
	public void calculateSourcesSinks
			(String sourceSinkFile) throws IOException, XmlPullParserException {
		PermissionMethodParser parser = PermissionMethodParser.fromFile(sourceSinkFile);
		Set<AndroidMethod> sources = new HashSet<AndroidMethod>();
		Set<AndroidMethod> sinks = new HashSet<AndroidMethod>();
		...
		if(sourceSinkManager!=null){
			sourceSinkManager.sourceMethods.clear();
			sourceSinkManager.sinkMethods.clear();
			for (AndroidMethod am : sources)
				sourceSinkManager.sourceMethods.put(am.getSignature(), am);
			for (AndroidMethod am : sinks)
				sourceSinkManager.sinkMethods.put(am.getSignature(), am);
		}
	}
	/**
	 * Calculates the sets of sources, sinks, entry points, and callbacks methods
